...
describe('restart restoration contracts',()=>{
 it('restores valid persisted selections after upgrade',()=>{const restored=migrateSettings({settingsVersion:1,selectedCharacter:'loko',sizeScale:3,activityLevel:'calm',alwaysOnTop:false});expect(restored.selectedCharacter).toBe('loko');expect(restored.walkingSpeed).toBe('calm');expect(restored.settingsVersion).toBe(3);});
