...
import { BehaviorPlanner, SeededRandom, createInitialMind, createSessionMemory, rememberActivity, rememberDisturbance, rememberRegion, updateMind, type ContextSummary, type PetIntention, type PetMind, type ScreenRegion, type SessionMemory } from '../../behavior/src/index.js';
...
export class LivingRuntimeController {
  ...
  private async executeIntention(i:PetIntention):Promise<void>{ this.previousMode=this.mode; this.generation+=1; this.activeId=i.kind; if(i.kind==='remain_idle'){this.mode='idle';this.schedulePlan(i.durationMs);}
    else if(i.kind==='walk'){this.mode='walking';this.memory=rememberRegion(this.memory,i.destinationRegion);await this.port.walkToRegion(i.destinationRegion);}
    else if(i.kind==='sleep'){this.mode='sleeping';await this.executeSleepCommands(this.sleep.handle({type:'REQUEST_SLEEP',trigger:'planner',plannedDurationMs:i.durationMs,monotonicMs:performance.now()}).commands);}
    else if(i.kind==='activity'){this.mode='activity';this.activeId=i.activityId;const result=this.activities.start({requestId:`activity:${this.generation}`,character:this.character,activityId:i.activityId,currentState:'stable.idle_front',currentPosture:'standing_front',nowMs:performance.now()});await this.executeActivityCommands(result.commands);}
  ...
  private async executeActivityCommands(commands:readonly ActivityCommand[]):Promise<void>{
    ...
    for(const c of commands){
      if(c.type==='PLAY_ANIMATION'){
        ...
        this.mode='idle'; this.activeId=undefined;
  ...
  private async pauseSafely():Promise<void>{
    ...
    this.mode='paused'; this.activeId=undefined; this.publish();
  }
  ...
  private async resumeSafely():Promise<void>{
    ...
    this.mode='idle'; this.activeId=undefined; this.schedulePlan(1_200); this.publish();
  }
  ...
  private async applyContextRestraint(reason: 'screen-locked'|'fullscreen-quiet'|'quiet-mode'): Promise<void> {
    ...
    if (this.mode === 'dragged' || this.mode === 'paused' || this.mode === 'sleeping') return;
  ...
  private invalidate(reason:string):void{this.previousMode=this.mode;this.generation+=1;this.nextPlanAtMonotonicMs=undefined;this.clearDeadline();if(this.plannerTimer)clearTimeout(this.plannerTimer);this.plannerTimer=null;this.port.log('Living runtime invalidated',{reason,generation:this.generation});}
  private clearDeadline():void{if(this.deadlineTimer)clearTimeout(this.deadlineTimer);this.deadlineTimer=null;}
  private stopTimers():void{if(this.plannerTimer)clearTimeout(this.plannerTimer);if(this.mindTimer)clearInterval(this.mindTimer);this.clearDeadline();this.plannerTimer=null;this.mindTimer=null;}
